...
namespace Prototype_Testing
{
    ...
    public class InitialiseerSysteemSteps
    {
        ...
        {
            _driver.GetGameManager().StartSessie(_driver.test, SoortSpel.PARTIJSPEL, new List<string> { _driver.partij }, _driver.klas, _driver.userId);
        }
        ...
        public void ThenMoetMijnSessieAangemaaktZijn()
        {
            ...
            Assert.Equal(lkrs.Klas.Naam, (_driver.klas));
            Assert.True(lkrs.UserId == _driver.userId);
            Assert.Equal(lkrs.test.id, (_driver.test));
        }
    }
}
